	<?php
/*
Template Name: Construct - Construction Template
...
try {
    if (!empty($_POST)) {
        ...
        if (!isset($_POST['g-recaptcha-response'])) {
            $dzRes['status'] = 0;
			$dzRes['msg'] = 'ReCaptcha ayarlanmadı.';
        ...
        if (!$response->isSuccess()) {
            $dzRes['status'] = 0;
			$dzRes['msg'] = 'ReCaptcha doğrulanamadı.';
			echo json_encode($dzRes);
			exit;
        }
		...
		if($_POST['dzToDo'] == 'Contact')
		{
			...
			{
				$dzRes['status'] = 0;
				$dzRes['msg'] = 'Hatalı E-posta Formatı.';
			}
			$dzMailSubject = 'Çetinler İnşaat|İletişim Formu: Bir Kişi İletişime Geçmek İstiyor';
			$dzMailMessage	= 	"
								Bir kişi sizinle iletişime geçmek istiyor: <br><br>
								İsim: $dzName<br/>
								E-posta: $dzEmail<br/>
								Mesaj: $dzMessage<br/>
								";
			...
			{
				$dzRes['status'] = 1;
				$dzRes['msg'] = 'Mesajınız başarıyla alındı. Bize ulaştığınız için teşekkürler.';
			}
			else
			{
				$dzRes['status'] = 0;
				$dzRes['msg'] = 'Posta gönderilirken bir sorun oluştu, lütfen daha sonra tekrar deneyin.';
			}
			echo json_encode($dzRes);
			exit;
		}
		...
		if($_POST['dzToDo'] == 'Appointment')
		{
			...
			$dzMailSubject = 'Çetinler İnşaat|Randevu Formu: Bir Kişi İletişime Geçmek İstiyor';
			$dzMailMessage	= 	"
								Bir kişi sizinle iletişime geçmek istiyor: <br><br>
								İsim: $dzName<br/>
								E-posta: $dzEmail<br/>
    ...
    $dzRes['status'] = 0;
	$dzRes['msg'] = $e->getMessage().'Posta gönderilirken bir sorun oluştu, lütfen daha sonra tekrar deneyin.';
	echo json_encode($dzRes);
	exit;
}
